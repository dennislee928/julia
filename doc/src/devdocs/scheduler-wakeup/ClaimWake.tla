...
(*     forever: a livelock (observed in practice as one spinner at ~10M      *)
(*     iterations/s with every other worker parked).                          *)
(*                                                                           *)
...
(* has an enabled step). The broken policy fails it with the observed        *)
(* cycle: the owner parks, all of its actions become disabled (fairness is   *)
(* vacuous for a parked thread), and the thief loops pop -> fail forever.    *)
(*                                                                           *)
...
CONSTANT ClaimPolicy    \* "mailbox" (fixed) or "reinject" (the bug)
